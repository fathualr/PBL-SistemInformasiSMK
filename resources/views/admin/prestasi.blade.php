    ...
    <div class="flex items-center">
        <div class="relative hidden md:flex mr-2">
            <select onchange="window.location.href=this.value" class="select border-b-2 border-base-300">
                <option value="{{ route('admin.prestasiSiswa.index', ['perPage' => 10]) }}"
                    {{ request()->get('perPage') == 10 ? 'selected' : '' }}>10</option>
                <option value="{{ route('admin.prestasiSiswa.index', ['perPage' => 25]) }}"
                    {{ request()->get('perPage') == 25 ? 'selected' : '' }}>25</option>
                <option value="{{ route('admin.prestasiSiswa.index', ['perPage' => 50]) }}"
                    {{ request()->get('perPage') == 50 ? 'selected' : '' }}>50</option>
                <option value="{{ route('admin.prestasiSiswa.index', ['perPage' => 75]) }}"
                    {{ request()->get('perPage') == 75 ? 'selected' : '' }}>75</option>
                <option value="{{ route('admin.prestasiSiswa.index', ['perPage' => 100]) }}"
        ...
        <form action="{{ route('PrestasiSiswa.store') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-5">
                <textarea
                    ...
                    id="editor" placeholder="Deskripsi Prestasi" name="deskripsi_prestasi"></textarea>
            </div>
            ...
            <button type="button" id="btnAddFilePrestasi"
                class="btn bg-blue-400 text-white w-full hover:text-blue-400 mt-5">Tambah
                Gambar</button>
    ...
    <div class="modal-box w-11/12 max-w-5xl py-0">
        ...
        <form action="{{ route('PrestasiSiswa.update',  $prestasi->id_prestasi) }}" method="post"
            ...
            <div class="mb-5">
                <textarea
                    class="input border-2 border-blue-400 flex items-center gap-2 mb-5 w-full focus-within:outline-none grow py-2"
                    id="editor2" placeholder="Deskripsi Prestasi"
